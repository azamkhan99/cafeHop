...
# Cache for GTFS data: dict = loaded, _GTFS_MISSING = file absent (do not re-log / re-stat every request)
_GTFS_MISSING = object()
_gtfs_cache: dict | None | object = None
...
def _load_gtfs_data(json_path=None):
    ...
    if _gtfs_cache is _GTFS_MISSING:
        return None
    if isinstance(_gtfs_cache, dict):
        return _gtfs_cache

    # Try to find JSON file
    if json_path is None:
        # Default: gtfs_precomputed.json next to this module (services/cafe/)
    ...
    if not os.path.exists(json_path):
        logger.warning("GTFS JSON not found at %s — closest-subway enrichment disabled.", json_path)
        _gtfs_cache = _GTFS_MISSING
        return None
    ...
    try:
        ...
        if file_size == 0:
            logger.warning("GTFS JSON file is empty: %s", json_path)
            _gtfs_cache = _GTFS_MISSING
            return None
        ...
        if not isinstance(data, dict) or not all(key in data for key in required_keys):
            logger.warning("GTFS JSON missing required keys: %s", json_path)
            _gtfs_cache = _GTFS_MISSING
            return None

        # Cache the loaded data
        _gtfs_cache = data
        logger.info("Loaded GTFS data from %s", json_path)
        return _gtfs_cache
    except json.JSONDecodeError as e:
        logger.warning("GTFS JSON decode error %s: %s", json_path, e)
        _gtfs_cache = _GTFS_MISSING
        return None
    except Exception as e:
        logger.exception("Error loading GTFS JSON %s", json_path)
        _gtfs_cache = _GTFS_MISSING
        return None
...
def get_closest_subway_station(lat, lon, json_path=None):
    ...
    line_list = [line.lower() for line in lines if "X" not in line]
    return {
...
def build_google_maps_link_nearby(cafe_name, lat, lon, radius=350):
    ...
    try:
        ...
        results = geocode_result.get("results") or []
        ...
        if not relevant_results:
            logger.warning(
                "Places nearby returned no cafe/bakery/food match for %r at (%s, %s)",
                cafe_name,
                lat,
                lon,
            )
        ...
        place_types = closest_place.get("types") or []
        ...
        logger.warning("Google Places nearby failed: %s", e)
        return "", ""