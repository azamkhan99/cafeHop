...
_nominatim_user_agent = (
    os.environ.get('NOMINATIM_USER_AGENT') or os.environ.get('NOMINATIM_API_KEY') or ''
).strip() or 'cafehop-lambda/1.0'
...
def _load_gtfs_data(json_path=None):
    ...
    if json_path is None:
        # Try in the same directory as utils.py (legacy/lambda/ directory)
